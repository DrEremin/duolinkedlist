...
public class DuoLinkedList <T> {
    ...
    DuoLinkedList(DuoLinkedList<T> other) {
        sizeList = 0;
        currentIndex = 0;
        start = null;
        current = null;
    ...
    /*public <T> T[] toArray() {
        T data = (T) start.getData().orElse(null);
        if ()

        T[] array = (T[]) Array.newInstance(start.getData().orElse(null).getClass(), sizeList);
        for (int i = 0; i < sizeList; i++) {
            array[i] = (T) getDataAtIndex(i).orElse(null);
        }
        return array;
    }*/
}
